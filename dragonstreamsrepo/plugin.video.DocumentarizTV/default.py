# -*- coding: utf-8 -*-
#------------------------------------------------------------
# http://www.youtube.com/user/DocumentarizTV
#------------------------------------------------------------
...
YOUTUBE_CHANNEL_ID = "DocumentarizTV"

# Entry point
def run():
    plugintools.log("DocumentarizTV.run")
...
def main_list(params):
    plugintools.log("DocumentarizTV.main_list "+repr(params))

    # On first page, pagination parameters are fixed
    if params.get("url") is None:
        params["url"] = "http://gdata.youtube.com/feeds/api/users/"+YOUTUBE_CHANNEL_ID+"/uploads?start-index=1&max-results=12"
    ...
    plugintools.add_item( action="main_list" , title="[COLOR=blue] Next Page --->>[/COLOR]" , url=next_page_url , folder=True )
